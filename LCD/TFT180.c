...
#include "COMMON.h"
#define TFT180_DC(x)                                                           \
  ((x) ? (DL_GPIO_setPins(TFT_PORT, TFT_DC_PIN))                               \
       : (DL_GPIO_clearPins(TFT_PORT, TFT_DC_PIN)))
#define TFT180_RST(x)                                                          \
  ((x) ? (DL_GPIO_setPins(TFT_PORT, TFT_RES_PIN))                              \
       : (DL_GPIO_clearPins(TFT_PORT, TFT_RES_PIN)))
#define TFT180_CS(x)                                                           \
  ((x) ? (DL_GPIO_setPins(TFT_PORT, TFT_CS_PIN))                               \
       : (DL_GPIO_clearPins(TFT_PORT, TFT_CS_PIN)))
#define TFT180_BL(x)                                                           \
  ((x) ? (DL_GPIO_setPins(TFT_PORT, TFT_BL_PIN))                               \
       : (DL_GPIO_clearPins(TFT_PORT, TFT_BL_PIN)))
...
void tft180_write_16bit_data(uint16_t data) {
  DL_SPI_transmitData8(TFT_SPI_INST, (uint8_t)((data & 0xFF00) >> 8));
  while (DL_SPI_isBusy(TFT_SPI_INST))
    ;
  DL_SPI_transmitData8(TFT_SPI_INST, (uint8_t)(data & 0x00FF));
  while (DL_SPI_isBusy(TFT_SPI_INST))
    ;
}
...
void tft180_show_char_color(uint8_t x, uint8_t y, const char dat,
                            uint16_t tft180_bgcolor, uint16_t tft180_pencolor) {
  uint8_t i, j;
  TFT180_CS(0);
  for (i = 0; i < 8; i++) {
...
void tft180_show_num_color(uint8_t x, uint8_t y, const float dat, uint8_t num,
  ...
  for (int i = 0; i < 17; i++)
    data_buffer[i] = 0;
  for (int i = 0; i < num + pointnum + 2; i++)
    data_buffer[i] = '\0';
...
void tft180_show_string_color(uint8_t x, uint8_t y, const char dat[],
  ...
  uint8_t j = 0;
  while (dat[j] != '\0') {
    tft180_show_char_color(x + 8 * j, y, dat[j], tft180_bgcolor,
                           tft180_pencolor);
    j++;
  }
}
